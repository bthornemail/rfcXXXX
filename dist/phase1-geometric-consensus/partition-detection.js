...
export class PartitionDetector {
     ...
     */
    detectViaBettiNumbers(vertices) {
     ...
     */
    decomposeGeometricType(original, partitionCount) {
     ...
     */
    getDefaultDecomposition(original, partitionCount) {
        ...
        if (verticesPerPartition >= 20)
            return GeometricType.DODECAHEDRON;
        if (verticesPerPartition >= 12)
            return GeometricType.ICOSAHEDRON;
        if (verticesPerPartition >= 8)
            return GeometricType.CUBE;
        if (verticesPerPartition >= 6)
            return GeometricType.OCTAHEDRON;
        if (verticesPerPartition >= 4)
            return GeometricType.TETRAHEDRON;
        if (verticesPerPartition >= 3)
            return GeometricType.TRIANGLE;
        if (verticesPerPartition >= 2)
     ...
     */
    findConnectedComponents(vertices, edges) {
        const components = [];
     ...
     */
    dfs(vertexId, adjacency, visited, component) {
     ...
     * Group vertices by partition
     */
    groupVerticesByPartition(vertices, components) {
        return components.map(component => vertices.filter(vertex => component.has(vertex.id)));
    }
    /**
     * Infer geometric type from vertex count
     */
    inferGeometricType(vertexCount) {
        const vertexToType = {
     ...
     */
    createPartitionCertificate(originalCertificate, partitionInfo) {
        return {
     ...
     */
    generateDecompositionProof(partitionInfo) {
...
This decomposition follows RFC XXXX Appendix I specifications.
     ...
     */
    analyzePartitionImpact(partitionInfo) {
        ...
        const requiredAgreementPerPartition = partitionVertices.map(partition => Math.ceil(partition.length * decomposedShape.threshold));
     ...
     */
    validatePartitionDetection(partitionInfo) {
        try {
...
//# sourceMappingURL=partition-detection.js.map