 ...
 */
import { GeometricType, getGeometricShape, getDual } from './geometric-types.js';
/**
...
export class DualPartitionRecovery {
     ...
     */
    recoverFromPartition(partitionCertificates, originalType) {
        try {
            ...
            const recoveryProof = this.generateRecoveryProof(partitionCertificates, originalShape, dualShape, unifiedConsensus);
            // Create recovered certificate
            const recoveredCertificate = this.createRecoveredCertificate(unifiedConsensus, originalType, partitionCertificates);
            return {
                success: true,
            ...
            return {
                success: false,
                recoveredCertificate: this.createErrorCertificate(originalType, error),
                recoveryProof: `Recovery failed: ${error.message}`,
                dualMapping: {
                    original: originalType,
     ...
     */
    mapThresholdViaDual(partitionThreshold, original, dual) {
     ...
     */
    createRecoveryPlan(partitionCertificates, originalType) {
        const originalShape = getGeometricShape(originalType);
        const dualShape = getDual(originalShape);
        ...
        const recoverySteps = this.generateRecoverySteps(partitionCertificates, originalShape, dualShape, recoveryStrategy);
     ...
     */
    validatePartitionCertificates(certificates) {
     ...
     * Apply dual mapping to unify threshold semantics
     */
    applyDualMapping(partitionCertificates, originalShape, dualShape) {
        ...
        const unifiedThreshold = this.mapThresholdViaDual(originalShape.threshold, originalShape.type, dualShape.type);
     ...
     */
    calculateThresholdMapping(original, dual) {
     ...
     * Generate recovery proof
     */
    generateRecoveryProof(partitionCertificates, originalShape, dualShape, unifiedConsensus) {
...
This recovery follows RFC XXXX Appendix I dual recovery specifications.
     ...
     * Create recovered certificate
     */
    createRecoveredCertificate(unifiedConsensus, originalType, partitionCertificates) {
        const originalShape = getGeometricShape(originalType);
     ...
     */
    createErrorCertificate(originalType, error) {
        const originalShape = getGeometricShape(originalType);
        return {
            certificateId: this.generateCertificateId(),
            geometricType: originalType,
     ...
     */
    determineRecoveryStrategy(partitionCertificates, _originalShape) {
        const partitionCount = partitionCertificates.length;
     ...
     * Generate recovery steps
     */
    generateRecoverySteps(partitionCertificates, originalShape, dualShape, strategy) {
        const steps = [];
        switch (strategy) {
            case 'dual':
                steps.push({
    ...
    estimateRecoveryTime(steps) {
        ...
        const complexityMultiplier = steps.reduce((sum, step) => {
            if (step.geometricType === GeometricType.TWENTY_FOUR_CELL)
                return sum + 2;
            if (step.geometricType === GeometricType.OCTAHEDRON)
                return sum + 1.5;
            if (step.geometricType === GeometricType.TETRAHEDRON)
                return sum + 1;
            return sum + 1;
        }, 0);
...
//# sourceMappingURL=dual-recovery.js.map