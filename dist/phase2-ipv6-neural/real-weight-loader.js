...
export class RealWeightLoader {
    ...
    async loadFromFile(filePath) {
        try {
            ...
            const weightFile = JSON.parse(fileContent);
            ...
            throw new Error(`Failed to load weights from file ${filePath}: ${error.message}`);
        }
    }
    /**
    ...
    async loadFromHTTP(url) {
        try {
            ...
            const weightFile = await response.json();
            // Validate file format
            if (!weightFile.weights || !weightFile.architecture) {
            ...
            throw new Error(`Failed to load weights from HTTP ${url}: ${error.message}`);
        }
    }
    /**
     * Save weights to a JSON file
     */
    async saveToFile(weights, filePath, architecture, description = '') {
        try {
            ...
            throw new Error(`Failed to save weights to file ${filePath}: ${error.message}`);
        }
    }
    /**
    ...
    async loadFromBase64(base64Data) {
        try {
            ...
            const weightFile = JSON.parse(jsonString);
            // Validate file format
            if (!weightFile.weights || !weightFile.architecture) {
     ...
     */
    async saveToBase64(weights, architecture, description = '') {
        try {
            // Convert Float32Array to regular arrays for JSON serialization
            const serializableWeights = {};
            for (const [key, value] of Object.entries(weights)) {
    ...
    async getFileInfo(filePath) {
        try {
            ...
            throw new Error(`Failed to read file info from ${filePath}: ${error.message}`);
        }
    }
}
//# sourceMappingURL=real-weight-loader.js.map