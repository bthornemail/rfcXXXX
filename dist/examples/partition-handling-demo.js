 ...
 */
export function runPartitionHandlingDemo() {
    ...
    const verticesForPartition = decisionVertices.map(v => ({ id: v.id, name: v.name, connected: new Set() }));
    ...
    const workflowDecisionVertices = [
    ...
    const workflowVertices = workflowDecisionVertices.map(v => ({ id: v.id, name: v.name, connected: new Set() }));
    ...
    const workflowRecovery = dualRecovery.recoverFromPartition([partitionAConsensusWorkflow.certificate, partitionBConsensusWorkflow.certificate], GeometricType.CUBE);
...
//# sourceMappingURL=partition-handling-demo.js.map