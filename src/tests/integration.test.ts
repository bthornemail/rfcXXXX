import { GeometricConsensus, DecisionVertex, ConsensusResult } from '../phase1-geometric-consensus/geometric-consensus.js';
...
export async function testIntegration(): Promise<boolean> {
  ...
  runTest('Error handling across components', () => {
    ...
    try {
      ...
      const invalidVotes: DecisionVertex[] = [
      ...
      const invalidArch: NeuralArchitecture = {
  ...
  runTest('Performance with large datasets', async () => {
    ...
    const largeVotes: DecisionVertex[] = Array.from({ length: 100 }, (_, i) => ({
      id: `node${i + 1}`,
      name: `Node ${i + 1}`,
    ...
    const architectures: NeuralArchitecture[] = Array.from({ length: 50 }, (_, i) => ({
      modelFamily: i % 3,
      featureDim: 100 + i * 10,
      hiddenLayers: 1 + (i % 10),
      attentionHeads: 1 + (i % 8),
      activation: ['relu', 'gelu', 'silu'][i % 3] as any,
      normalization: ['layer', 'batch', 'rms'][i % 3] as any,
  ...
  runTest('Memory management', async () => {
    ...
    const models: BrowserModelRuntime[] = [];
    ...
    return Promise.all(loadPromises)
      .then(() => {
        ...
        const inferencePromises = models.map(model =>
          model.forwardPass([1.0, 2.0])
        );

        return Promise.all(inferencePromises);
      })
      .then((outputs: ForwardPassResult[]) => {
  ...
  runTest('Concurrent operations', async () => {
    ...
    return Promise.all([...consensusPromises, ...encodingPromises])
      .then((results) => {
        ...
        return consensusResults.every((result: any) => result.success) &&
               encodingResults.every((result: string | ConsensusResult) =>
                 typeof result === 'string' ? result.includes(':') : true);
      })
      .catch(() => false);
  });
