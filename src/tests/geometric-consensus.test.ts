 ...
 */
export function testGeometricConsensus(): boolean {
  ...
  runTest('24-Cell successful consensus', () => {
    const votes: DecisionVertex[] = Array.from({ length: 24 }, (_, i) => ({
      id: `fed${i + 1}`,
      name: `Fed ${i + 1}`,
  ...
  runTest('Truncated Tetrahedron unanimous consensus', () => {
    const votes: DecisionVertex[] = Array.from({ length: 12 }, (_, i) => ({
      id: `global${i + 1}`,
      name: `Global ${i + 1}`,
      agrees: true
    }));
  ...
  // Test 12: Single vote
  runTest('Single vote', () => {
    const votes: DecisionVertex[] = [{ id: 'single', name: 'Single', agrees: true }];
    const result = consensus.mustLocal(votes);
